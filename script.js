...
// Event delegation for add to cart and favorites (skip on favorites page)
document.addEventListener('click', (e) => {
    // Skip if on favorites page to prevent double adding
...
// UI Toggle Functions (with null checks)
...
if (cartIcon && cartItemsContainer) {
    cartIcon.addEventListener("click", () => {
        ...
        if (receiptContainer) receiptContainer.classList.remove("active");
...
function addToCart(name, price, image) {
    ...
    updateCartDisplay();
    if (cartItemsContainer) cartItemsContainer.classList.add('active');
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Receipt toggle (with null check)
    const receiptIcon = document.querySelector('.receipt-icon');
    if (receiptIcon) {
    ...
    // Tracking toggle (with null check)
    const trackingIcon = document.querySelector('.tracking-icon');
    if (trackingIcon) {
    ...
    // Admin panel access (with null check)
    const adminIcon = document.querySelector('.admin-icon');
    if (adminIcon) {
    ...
    // Authentication (with null check)
